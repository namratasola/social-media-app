...
const SocketHandler = (socket, io) => {
  ...
  socket.on('postLiked', async ({userId ,postId}) =>{
        try {
            ...
            io.to(post.userId).emit('notification', notification); // Notify the post owner
            socket.emit("likeUpdated");
        } catch (error) {
            console.error('Error handling postLiked:', error);
        }
    });
...
socket.on('update-messages', async ({ chatId }) => {
  ...
  socket.on('new-message', async ({ chatId, id, text, file, senderId, date }) => {
      try {
          ...
          const receiverId = chat._id.replace(senderId, ''); // assuming chatId is a combination of senderId and receiverId
          const notification = {
              type: 'chat',
              senderId,
              chatId,
          ...
          io.to(receiverId).emit('notification', notification); // Notify the chat receiver
          socket.emit('messages-updated', { chat });
  ...
  // Join user to a room for receiving notifications
...
socket.on('join', (userId) => {
  // Check if the user is already in the room to avoid multiple joins
